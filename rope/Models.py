...
class Models():
    ...
    def run_detect(self, img, detect_mode='Retinaface', max_num=1, score=0.5):
        ...
        if detect_mode=='Retinaface':
            if not self.retinaface_model:
                print("now begin load det_10g.onnx")      
                self.retinaface_model = onnxruntime.InferenceSession(self.modelPath+'models/det_10g.onnx', providers=self.providers)
                print("now loaded det_10g.onnx")      
            ...
            if not self.scrdf_model:
                self.scrdf_model = onnxruntime.InferenceSession(self.modelPath+'models/scrfd_2.5g_bnkps.onnx', providers=self.providers)
            ...
            if not self.yoloface_model:
                self.yoloface_model = onnxruntime.InferenceSession(self.modelPath+'models/yoloface_8n.onnx', providers=self.providers)
                self.insight106_model = onnxruntime.InferenceSession(self.modelPath+'models/2d106det.onnx', providers=self.providers)
    ...
    def run_recognize(self, img, kps):
        if not self.recognition_model:
            print("now begin load w600k_r50.onnx")            
            self.recognition_model = onnxruntime.InferenceSession(self.modelPath+'models/w600k_r50.onnx', providers=self.providers)
            print("now loaded w600k_r50.onnx")
    ...
    def run_swap_stg1(self, embedding):
        ...
        if not self.swapper_model_kps:
            self.swapper_model_kps = onnxruntime.InferenceSession( self.modelPath+"models/inswapper_kps.onnx", providers=self.providers)
    ...
    def run_swap_stg2(self, image, holder, output):
        if not self.swapper_model_swap:
            self.swapper_model_swap = onnxruntime.InferenceSession( self.modelPath+"models/inswapper_swap.onnx", providers=self.providers)
    ...
    def run_GFPGAN(self, image, output):
        if not self.GFPGAN_model:
            ...
            # self.GFPGAN_model = onnxruntime.InferenceSession( self.modelPath+"models/GFPGANv1.4.onnx", sess_options, providers=[("CUDAExecutionProvider", cuda_options), 'CPUExecutionProvider'])
            print("loading GFPGANv1.4.onnx")
            self.GFPGAN_model = onnxruntime.InferenceSession( self.modelPath+"models/GFPGANv1.4.onnx", providers=self.providers)
            print("GFPGANv1.4.onnx loaded")
    ...
    def run_GPEN_512(self, image, output):
        if not self.GPEN_512_model:
            print("loading GPEN-BFR-512.onnx")
            self.GPEN_512_model = onnxruntime.InferenceSession( self.modelPath+"models/GPEN-BFR-512.onnx", providers=self.providers)
            print("GPEN-BFR-512.onnx loaded")
    ...
    def run_GPEN_256(self, image, output):
        if not self.GPEN_256_model:
            self.GPEN_256_model = onnxruntime.InferenceSession( self.modelPath+"models/GPEN-BFR-256.onnx", providers=self.providers)
    ...
    def run_codeformer(self, image, output):   
        if not self.codeformer_model:    
            print("loading codeformer_fp16.onnx")
            self.codeformer_model = onnxruntime.InferenceSession( self.modelPath+"models/codeformer_fp16.onnx", providers=self.providers)
            print("codeformer_fp16.onnx loaded")
    ...
    def run_occluder(self, image, output):    
        if not self.occluder_model:
            print("loading occluder.onnx")
            self.occluder_model = onnxruntime.InferenceSession(self.modelPath+"models/occluder.onnx", providers=self.providers)
            print("occluder.onnx loaded")
    ...
    def run_faceparser(self, image, output):    
        if not self.faceparser_model:
            self.faceparser_model = onnxruntime.InferenceSession(self.modelPath+"models/faceparser_fp16.onnx", providers=self.providers)
    ...
    def resnet50(self, image, score=.5):   
        if not self.resnet50_model:
            self.resnet50_model = onnxruntime.InferenceSession(self.modelPath+"models/res50.onnx", providers=self.providers)
